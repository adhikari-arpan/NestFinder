...
export const LandlordDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (params.get("action") === "post") {
      ...
      if (currentUser?.role === "landlord" && !currentUser.is_verified) {
        const timeoutId = setTimeout(() => setGateNotice(true), 0);
        return () => clearTimeout(timeoutId);
      } else {
        const timeoutId = setTimeout(() => setIsPostModalOpen(true), 0);
        return () => clearTimeout(timeoutId);
      }
    }
  ...
  const handlePostSubmit = async (e) => {
    ...
    const payload = {
      ...
      city: formCity,
      latitude: Number(formLat) || 27.685,
      longitude: Number(formLng) || 85.32,
      images: formImages,
    ...
    <div className="animate-fade-in dashboard-container">
        ...
        <>
          ...
          {currentUser?.role === "landlord" && (
            <KycStatusBanner
              ...
              rejectionReason={myKyc?.rejection_reason}
            />
          )}
      ...
      {!isPostModalOpen && (
        ...
        >
          ...
          >
            <MessageSquare size={16} /> Tenant Inquiries (
            {landlordInquiries.length})
          </button>
        </div>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {landlordListings.length === 0 ? (
            <div className="card text-center" style={{ padding: "4rem 1rem" }}>
              ...
              >
                Submit your first room details and get matched with tenant
                search lists.
              </p>
              <button
          ...
          {landlordInquiries.length === 0 ? (
            <div className="card text-center" style={{ padding: "4rem 1rem" }}>
              ...
              <p style={{ color: "var(--text-muted)" }}>
                Tenants who view your listing will fill in a contact form and
                their messages will display here.
              </p>
            </div>
          ) : (
            landlordInquiries.map((inq) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    Inquiry on: <strong>{matchedRoom?.title}</strong> (Rs.{" "}
                    {matchedRoom?.price.toLocaleString()}/mo)
                  </div>
                  ...
                  {inq.status === "replied" ? (
                    ...
                    >
                      <strong
                        style={{ color: "var(--secondary)", display: "block" }}
                      >
                        ✓ You Replied:
                      </strong>
          ...
          <div style={{ maxWidth: "800px", margin: "0 auto" }}>
            ...
            >
              <div>
                ...
                >
                  <Plus style={{ color: "var(--primary)" }} size={24} />{" "}
                  {editingListing
                    ? "Edit Room / Flat Listing"
                    : "Add Room / Flat Listing"}
                </h2>
                <p
                  style={{
                    fontSize: "0.85rem",
            ...
            >
              ...
              >
                <strong style={{ color: "var(--text-main)" }}>
                  Listings are visible to tenants for {LISTING_VISIBILITY_DAYS}{" "}
                  days.
                </strong>{" "}
                Once admin verifies your listing, it stays live in search
                results for {LISTING_VISIBILITY_DAYS} days from that point.
                After that it's automatically removed — check the "Days
                Remaining" column on your listings table.
              </p>
            </div>
              ...
              <div className="card" style={{ padding: "1.5rem" }}>
                ...
                {myKyc ? (
                  <>
                    ...
                    >
                      This is set from your verified KYC address and can't be
                      edited here. To post from a different location,{" "}
                      <Link
                        to="/kyc"
                        style={{ color: "var(--primary)", fontWeight: 600 }}
                      >
                        update your KYC verification
                      </Link>{" "}
                      — this will require re-approval before you can post again.
                    </p>
                  </>
                ) : (
                  <p
          ...
          >
            ...
            >
              {confirmedAction === "updated" ? (
                ...
                <>
                  "{confirmedListing.title}" has been posted and is{" "}
                  <strong>pending admin review</strong>. You'll get a
                  notification once it's verified and live for tenants to see.
                </>
              )}
            </p>
            <div
      ...
      {listingToDelete && (
        ...
        >
          ...
          >
            ...
            >
              "{listingToDelete.title}" and all its photos, inquiries, and
              reviews will be permanently removed. This can't be undone.
            </p>
            {deleteError && (
              <p
                style={{
