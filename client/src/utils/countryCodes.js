// ITU-T international dialing codes. Nepal is pinned first since NestFinder
// is a Nepal-only rental platform and it's the overwhelmingly common choice;
// everything else is listed alphabetically for the search dropdown.
...
// no image assets or icon font needed.
...
// Nepali mobile numbers: exactly 10 digits, starting 96/97/98 (the current
...
export function validatePhoneNumber(dial, nationalNumber) {
  ...
  if (dial === '977') {
    if (!NEPAL_MOBILE_RE.test(digits)) {
      return 'Enter a valid 10-digit Nepali mobile number (starts with 96, 97, or 98).';
    }
    return null;
  }

  if (digits.length < 6 || digits.length > 14) {
    return 'Enter a valid phone number (6-14 digits).';
  }
  return null;
}
