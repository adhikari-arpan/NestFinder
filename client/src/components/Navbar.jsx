...
export const Navbar = () => {
  ...
  const computeNotifPos = () => {
    ...
    setNotifPos({
      ...
      maxHeight: Math.min(
        NOTIF_MAX_HEIGHT,
        window.innerHeight - top - NOTIF_VIEWPORT_MARGIN,
      ),
    });
  };
  ...
  useEffect(() => {
    if (notifOpen) {
      // Defer state updates to the next animation frame to avoid
      // synchronous setState inside the effect which can trigger
      // cascading renders.
  ...
  return (
    <nav className="glass sticky-nav sticky top-0 z-1000 border-b border-(--border-color) py-3 transition-[background] duration-(--transition-normal)">
      <div className="relative container flex flex-nowrap items-center justify-between">
        ...
        >
          ...
          />
          <span>
            Nest<span className="text-(--text-main)">Finder</span>
          </span>
        </Link>
        ...
        {currentUser && (
          <div className="hidden items-center gap-6 md:flex">
            {currentUser.role === "tenant" && (
              <Link
                to="/dashboard/tenant"
                className={navLinkClass("/dashboard/tenant")}
              >
                Tenant Hub
              </Link>
            )}
            {currentUser.role === "landlord" && (
              <Link
                ...
                className={navLinkClass("/dashboard/landlord")}
              >
                Landlord Hub
              </Link>
            )}
              ...
              {notifMounted &&
                createPortal(
                  <>
                    ...
                    <div
                      className={`card flex w-[320px] flex-col overflow-hidden border border-(--border-color) p-0 shadow-xl transition-all duration-150 ease-out ${
                        notifVisible
                    ...
                    >
                      ...
                      <div
                        ref={notifListRef}
                        className="flex min-h-0 flex-1 flex-col gap-1.5 overflow-y-auto p-4 pt-2"
                      >
                        {notifications.length === 0 ? (
                          <p className="p-4 text-center text-[0.85rem] text-(--text-muted)">
                            No notifications
                          </p>
                        ) : (
                          notifications.map((n) => {
                            ...
                            return (
                              <div
                                key={n.id}
                                onClick={() =>
                                  !n.read && markNotificationAsRead(n.id)
                                }
                                className={`flex items-start gap-2.5 rounded-md border-l-[3px] p-3 transition-colors hover:bg-(--bg-app) ${
                                  n.read
        ...
        {mobileMenuOpen && (
          <div className="glass animate-fade-in rounded-md) absolute inset-x-0 top-full z-50 mt-2 flex flex-col gap-3 border border-(--border-color) bg-(--bg-card) p-4 shadow-(--shadow-lg)">
            {currentUser?.role === "tenant" && (
              <Link
                to="/dashboard/tenant"
                onClick={() => setMobileMenuOpen(false)}
                className="rounded-sm) px-2 py-1.5 font-medium hover:bg-(--primary-light) hover:text-(--primary)"
              >
                My Dashboard
              </Link>
            )}
              ...
              <>
                ...
                <button
                  onClick={() => {
                    ...
                    navigate("/dashboard/landlord?action=post");
                  }}
