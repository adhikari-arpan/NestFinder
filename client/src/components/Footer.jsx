...
export const Footer = () => {
  ...
  return (
    <footer
      className="mt-auto border-t border-(--border-color) bg-(--bg-card)"
      ...
      <div className="container">
        ...
        <div className="mb-8 flex flex-wrap items-start justify-between gap-10">
          ...
          <div className="flex max-w-100 flex-col gap-3">
            <Link
              to="/"
              className="flex items-center gap-2 text-[1.3rem] font-extrabold text-(--primary)"
            >
              <img
                src={logo}
                alt="NestFinder"
            ...
            <p className="m-0 text-[0.85rem] leading-[1.8] text-(--text-muted)">
              Nepal's leading map-based platform helping students and tenants
              discover verified room and flat rentals, powered by smart AI
              search algorithms.
            </p>
            <div className="mt-1 flex gap-2.5">
              {[Globe, Share2, MessageSquare].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="rounded-sm)] flex size-8 items-center justify-center bg-(--border-color) text-(--text-main) transition-all hover:-translate-y-0.5 hover:bg-(--primary) hover:text-white"
                >
                  <Icon size={15} />
                </a>
              ))}
            ...
            <div className="flex flex-col gap-2.5">
              ...
              {role === "tenant" && (
                <>
                  ...
                  <Link to="/ai-recommend" className={linkClass}>
                    AI Recommend
                  </Link>
                </>
              )}
              ...
              <Link to="/about/payment" className={linkClass}>
                Payment &amp; Pricing
              </Link>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-(--border-color) pt-5">
          <span className="text-[0.8rem] text-(--text-light)">
            &copy; {new Date().getFullYear()} NestFinder. Developed by Purnima
